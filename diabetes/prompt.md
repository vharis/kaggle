...
2. **API Endpoint:** 
   - Using FastAPI (or Flask), expose this model via a public-ready POST endpoint `/predict`.
   - The endpoint must accept a JSON payload with `age`, `bmi`, `glucose`, and `diabetic_pedigree_function`.
   - It should return a JSON response containing the binary prediction (0 or 1) and the confidence probability.
   - Include a fallback check: if `model.pkl` is missing, use a simple logical rule-base so the server doesn't crash during initial environment tests.
